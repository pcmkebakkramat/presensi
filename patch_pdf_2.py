...
pdf_logic = """
    ...
    function generatePdfFooter(doc, isMassal = false) {
      ...
      if (isMassal) {
        ...
        // Ketua PCM Kanan, Pembuat di kiri hilang
        doc.text(tglCetak, pageWidth - 15, finalY, { align: "right" });
        doc.text("Mengetahui,", pageWidth - 15, finalY + 5, { align: "right" });
        doc.text("Ketua PCM Kebakkramat", pageWidth - 15, finalY + 10, { align: "right" });
...
content = pattern.sub(pdf_logic, content)

# Tambahkan cleanString
clean_string_logic = """
...
"""
content = content.replace("// Format string URL agar cocok dengan DB", clean_string_logic + "\n    // Format string URL agar cocok dengan DB")
